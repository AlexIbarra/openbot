...
// pin 18 pwmvalue 250(para atras) 450(parado) 650 (para alante)
const int pwmPinD = 13; // PWM LED - Broadcom pin 18, P1 pin 12
const int pwmPinI = 18;
int pwmValue = 1024;

void giraMDDelante() {
	pwmWrite(pwmPinD, 310);
}

void giraMDAtras() {
	pwmWrite(pwmPinD, 510);
}

void giraMIDelante() {
	pwmWrite(pwmPinI, 510);
}

void giraMIAtras() {
	pwmWrite(pwmPinI, 310);
}

void pararMD() {
	pwmWrite(pwmPinD, 450);
}

void giraMI() {
	pwmWrite(pwmPinI, 450);
}
 ...
 int main( int argc, char** argv )
 {
    ...
    pinMode(pwmPinD, PWM_OUTPUT); // Set PWM LED as PWM output
	pinMode(pwmPinI, PWM_OUTPUT);
